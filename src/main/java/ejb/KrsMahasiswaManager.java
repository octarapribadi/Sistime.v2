...
public class KrsMahasiswaManager {
    ...
    public void toeng(List<User> users) {
        TypedQuery<KrsMahasiswa> query = em.createQuery("select k from KrsMahasiswa k " +
                        "join fetch k.skedul s " +
                        "join fetch s.idMatakuliah m " +
                        "where k.user in :users and m.id=197", KrsMahasiswa.class)
                .setParameter("users", users);
        List<KrsMahasiswa> krsMahasiswas = query.getResultList();
        krsMahasiswas.forEach(k -> System.out.println(k.getSkedul().getIdMatakuliah().getId() + ": " + k.getSkedul().getIdMatakuliah().getNamaMatakuliah()));
    }
